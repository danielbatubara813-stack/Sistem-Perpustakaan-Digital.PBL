...
class AnggotaController extends Controller
{
    public function indexAnggota(Request $request)
    {
        ...
        // Filter verifikasi admin (Menunggu / Terverifikasi / Ditolak)
        ...
        if ($request->sort == 'terlama') {
            $query->orderByRaw("FIELD(verifikasi_admin, '" . Anggota::VERIFIKASI_MENUNGGU . "') DESC")
        ...
        $anggotaMenunggu = Anggota::where('verifikasi_admin', Anggota::VERIFIKASI_MENUNGGU)->count();
    ...
    public function updateAnggota(Request $request, $id)
    {
        ...
        // Auto ubah status berdasarkan verifikasi
        if ($request->verifikasi_admin == Anggota::VERIFIKASI_TERVERIFIKASI) {
...
public function destroyAnggota(Request $request)
{
    try {
        ...
        // Cek apakah ada anggota yang masih menunggu verifikasi
        $menunggu = Anggota::whereIn('id_anggota', $ids)
            ->where('verifikasi_admin', Anggota::VERIFIKASI_MENUNGGU)
            ->count();

        if ($menunggu > 0) {
            return redirect()->back()->with('error',
                $menunggu . ' data tidak dapat dihapus karena masih menunggu verifikasi admin.');
        }
        ...
        if ($e->getCode() == 23000) {
            return redirect()->back()->with('error',
                'Data anggota gagal dihapus karena masih memiliki data yang terkait.');
        }
