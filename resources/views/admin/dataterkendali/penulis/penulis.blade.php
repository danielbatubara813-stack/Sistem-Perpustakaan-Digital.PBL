...
@section('content')
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <form method="GET" action="">
                    <div
                        class="bg-slate-100 rounded-md p-2 flex flex-wrap md:flex-nowrap items-center gap-2 w-full md:w-max">

                        <input name="search" value="{{ request('search') }}" type="text" placeholder="Cari penulis..."
                            class="w-full sm:w-auto sm:flex-1 sm:max-w-36 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200" />

                        <select name="tipe_penulis" id=""
                            class="w-full md:w-48 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200">
                            <option value="">Semua</option>
                            <option value="Nama Orang" {{ request('tipe_penulis') == 'Nama Orang' ? 'selected' : '' }}>
                                Nama
                                Orang</option>
                            <option value="Badan Organisasi"
                                {{ request('tipe_penulis') == 'Badan Organisasi' ? 'selected' : '' }}>
                                Badan Organisasi</option>
                            <option value="Konferensi" {{ request('tipe_penulis') == 'Konferensi' ? 'selected' : '' }}>
                                Konferensi</option>
                        </select>
    ...
    <div class="bg-white p-6 rounded-lg mt-4 shadow-lg">
        @if (count($penulis) > 0)
            <form action="{{ route('admin.data-terkendali.penulis.destroy') }}" id="multi-delete-form"
                data-delete-name="id_penulis" method="POST">
                @method('DELETE')
                @csrf
                        ...
                        <tbody>
                            @foreach ($penulis as $data)
                                <tr
                                    ...
                                    <td class="px-6 py-4">
                                        <input type="checkbox" name="id_penulis[]" value="{{ $data->id_penulis }}"
                                            class="row-checkbox h-5 w-5 rounded-full border-slate-300 text-blue-600 focus:ring-blue-500" />
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $data->nama_penulis }}</td>
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $data->tipe_penulis }}</td>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ route('admin.data-terkendali.penulis.edit', $data->id_penulis) }}"
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-500">
                        Menampilkan {{ $penulis->firstItem() ?? 0 }} hingga {{ $penulis->lastItem() ?? 0 }}
                        dari {{ $penulis->total() }} data
                    </p>
                    @if ($penulis->lastPage() > 1)
                        <div class="inline-flex items-center gap-2">
                            <div class="px-4 py-2 rounded-md text-sm text-slate-700">
                                Halaman <span class="font-semibold">{{ $penulis->currentPage() }}</span>
                                dari <span class="font-semibold">{{ $penulis->lastPage() }}</span>
                            </div>
                            <div class="flex items-center justify-center rounded-md gap-2 bg-slate-100 p-1">
                                ...
                                @else
                                    <a href="{{ $penulis->previousPageUrl() }}"
                                ...
                                @php
                                    $currentPage = $penulis->currentPage();
                                    $lastPage = $penulis->lastPage();
                                ...
                                @foreach ($penulis->getUrlRange($start, $end) as $page => $url)
