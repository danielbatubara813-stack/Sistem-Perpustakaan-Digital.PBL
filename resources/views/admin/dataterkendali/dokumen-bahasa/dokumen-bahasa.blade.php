...
@section('title', 'Kelola Dokumen Bahasa')
@php
...
@section('content')
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <form method="GET" action="">
                    <div class="bg-slate-100 rounded-md p-2 flex flex-wrap items-center gap-2 w-full md:w-max">
                        ...
                        <input name="search" value="{{ request('search') }}" type="text" placeholder="Cari bahasa..."
    ...
    <div class="bg-white p-6 rounded-lg mt-4 shadow-lg">
        @if (count($bahasa) > 0)
            <form action="{{ route('admin.data-terkendali.dok-bahasa.destroy') }}" id="multi-delete-form"
                data-delete-name="kode_bahasa" method="POST">
                @method('DELETE')
                @csrf
                <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h2 class="text-lg font-semibold tracking-wide">{{ count($bahasa) }} Daftar bahasa</h2>
                    </div>
                    ...
                    <table class="min-w-full text-sm text-left text-gray-600 text-nowrap">
                        <thead class="text-xs text-gray-600 uppercase bg-gray-300">
                            <tr>
                                ...
                                <th class="px-6 py-3">Kode bahasa</th>
                                <th class="px-6 py-3">Nama bahasa</th>
                        ...
                        <tbody>
                            @foreach ($bahasa as $item)
                                <tr
                                    ...
                                    <td class="px-6 py-4">
                                        <input type="checkbox" name="kode_bahasa[]" value="{{ $item->kode_bahasa }}"
                                            class="row-checkbox h-5 w-5 rounded-full border-slate-300 text-blue-600 focus:ring-blue-500" />
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $item->kode_bahasa }}</td>
                                    <td class="px-6 py-4 font-medium text-gray-900">{{ $item->nama_bahasa }}</td>
                                    <td class="px-6 py-4 hidden lg:table-cell">{{ $item->tanggal_dibuat }}</td>
                                    <td class="px-6 py-4 hidden lg:table-cell">{{ $item->tanggal_diubah }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ route('admin.data-terkendali.dok-bahasa.edit', $item->kode_bahasa) }}"
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-500">
                        Menampilkan {{ $bahasa->firstItem() ?? 0 }} hingga {{ $bahasa->lastItem() ?? 0 }}
                        dari {{ $bahasa->total() }} data
                    </p>
                    @if ($bahasa->lastPage() > 1)
                        <div class="inline-flex items-center gap-2">
                            <div class="px-4 py-2 rounded-md text-sm text-slate-700">
                                Halaman <span class="font-semibold">{{ $bahasa->currentPage() }}</span>
                                dari <span class="font-semibold">{{ $bahasa->lastPage() }}</span>
                            </div>
                            <div class="flex items-center justify-center rounded-md gap-2 bg-slate-100 p-1">
                                @if ($bahasa->onFirstPage())
                                ...
                                @php
                                    $currentPage = $bahasa->currentPage();
                                    $lastPage = $bahasa->lastPage();
                                ...
                                @foreach ($bahasa->getUrlRange($start, $end) as $page => $url)
                                    @if ($page == $bahasa->currentPage())
                                        <span
                                ...
                                @if ($bahasa->hasMorePages())
                                    <a href="{{ $bahasa->nextPageUrl() }}"
                                        class="p-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round">
