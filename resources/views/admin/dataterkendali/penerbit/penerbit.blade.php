            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <form method="GET" action="">
                    <div class="bg-slate-100 rounded-md p-2 flex flex-wrap items-center gap-2 w-full md:w-max">
                        ...
                        <input name="search" value="{{ request('search') }}" type="text" placeholder="Cari penerbit..."
            ...
            <form action="{{ route('admin.data-terkendali.penerbit.destroy') }}" id="multi-delete-form"
                ...
                <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-500">
                        Menampilkan {{ $penerbit->firstItem() ?? 0 }} hingga {{ $penerbit->lastItem() ?? 0 }}
                        dari {{ $penerbit->total() }} data
                    </p>
                    @if ($penerbit->lastPage() > 1)
                        <div class="inline-flex items-center gap-2">
                            <div class="px-4 py-2 rounded-md text-sm text-slate-700">
                                Halaman <span class="font-semibold">{{ $penerbit->currentPage() }}</span>
                                dari <span class="font-semibold">{{ $penerbit->lastPage() }}</span>
                            </div>
