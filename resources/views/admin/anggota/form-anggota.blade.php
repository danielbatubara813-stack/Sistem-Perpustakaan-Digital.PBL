...
@php
    ...
    $description =
        'Form untuk menambahkan data anggota baru ke dalam sistem perpustakaan, termasuk informasi identitas dan keanggotaan.';
@endphp
@section('content')
    ...
    <div class="bg-white p-6 rounded-lg mt-4 shadow-lg">
        <div class="mb-4 flex items-center justify-between">
            <div class="bg-slate-100 rounded-md px-2 py-1 flex items-center gap-2">
                <a href="{{ route('admin.anggota.daftar') }}"
                    class="px-4 py-2 text-sm {{ request()->routeIs('admin.anggota.create') ? 'bg-blue-600 text-white shadow rounded' : 'text-slate-600' }}">
                    Daftar Anggota
        ...
        @if (isset($anggota))
            <form method="POST" action="{{ route('admin.anggota.update', $anggota->id_anggota) }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-start">
            ...
            <div class="sm:col-span-9">
                <input name="identity_number" value="{{ old('identity_number', $anggota->nomor_identitas ?? '') }}"
                    type="text"
                    class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200" />
            ...
            <div class="sm:col-span-9">
                <input name="name" value="{{ old('name', $anggota->nama ?? '') }}" type="text"
            ...
            <div class="sm:col-span-9">
                <input name="registration_date" value="{{ old('registration_date', $anggota->tanggal_daftar ?? '') }}"
                    type="date"
                    class="rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200" />
            ...
            <div class="sm:col-span-9 flex items-center gap-6">
                ...
                @endphp
                <label class="inline-flex items-center gap-2">
                    <input type="radio" name="gender" value="L" {{ $selectedGender == 'L' ? 'checked' : '' }}
                        class="h-4 w-4 text-blue-600" /> Laki-laki
                </label>
                <label class="inline-flex items-center gap-2">
                    <input type="radio" name="gender" value="P" {{ $selectedGender == 'P' ? 'checked' : '' }}
                        class="h-4 w-4 text-blue-600" /> Perempuan
                </label>
                @error('gender')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            ...
            @if (isset($anggota))
                ...
                <div class="sm:col-span-9">
                    <select name="status_anggota"
                        class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-200">
                        <option value="Aktif"
                            {{ old('status_anggota', $anggota->status_anggota) == 'Aktif' ? 'selected' : '' }}>Aktif
                        </option>
                        <option value="Tidak Aktif"
                            {{ old('status_anggota', $anggota->status_anggota) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak
                            Aktif</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-9">
                    <select name="verifikasi_admin"
                        ...
                        <option value="Menunggu"
                            {{ old('verifikasi_admin', $anggota->verifikasi_admin) == 'Menunggu' ? 'selected' : '' }}>
                            Menunggu</option>
                        <option value="Terverifikasi"
                            {{ old('verifikasi_admin', $anggota->verifikasi_admin) == 'Terverifikasi' ? 'selected' : '' }}>
                            Terverifikasi</option>
                        <option value="Ditolak"
                            {{ old('verifikasi_admin', $anggota->verifikasi_admin) == 'Ditolak' ? 'selected' : '' }}>
                            Ditolak</option>
                    </select>
                </div>
            @endif
            ...
            <div class="sm:col-span-9">
                <div class="flex flex-col md:flex-row gap-4 items-start">
                    <div
                        ...
                        @if (isset($anggota) && $anggota->foto_ktp)
                            <img id="ktpPreview" src="{{ asset('storage/' . $anggota->foto_ktp) }}" alt="KTP"
                ...
                <div class="flex-1">
                    <input id="ktpInput" name="ktp_photo" type="file" accept=".jpg,.jpeg,.png,image/jpeg,image/png"
                        class="text-sm" />
                    @error('ktp_photo')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-slate-500 mt-1">Maks 10 MB. Format: jpg, jpeg,
                        png.{{ isset($anggota) ? ' Kosongkan jika tidak ingin mengganti.' : '' }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-start">
                <div
                    ...
                    @if (isset($anggota) && $anggota->profile)
                        <img id="profilePreview" src="{{ asset('storage/' . $anggota->profile) }}" alt="Profil"
                            class="object-contain w-full h-full" />
