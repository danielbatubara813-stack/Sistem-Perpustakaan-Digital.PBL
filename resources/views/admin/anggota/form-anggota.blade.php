        ...
        <form method="POST" action="{{ route('admin.anggota.store') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-start">
                ...
                <div class="sm:col-span-9 flex items-center gap-6">
                    <label class="inline-flex items-center gap-2"><input type="radio" name="identity_type" value="NIM"
                            {{ old('identity_type', 'NIM') == 'NIM' ? 'checked' : '' }} class="h-4 w-4 text-blue-600" />
                        NIM</label>
                    <label class="inline-flex items-center gap-2"><input type="radio" name="identity_type" value="NIDN"
                            {{ old('identity_type') == 'NIDN' ? 'checked' : '' }} class="h-4 w-4 text-blue-600" />
                        NIDN</label>
                    @error('identity_type')
                        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                    @enderror
