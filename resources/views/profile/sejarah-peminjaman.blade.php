...
@section('content')
    <div class="w-full flex flex-col gap-4 p-4">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2">
            <p>Terdapat <strong>{{ $peminjaman->count() }}</strong> sejarah peminjaman...</p>
            ...
            <div
                ...
                <a href="{{ route('detail-buku-page', $item->itemBuku->buku->id_buku) }}">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-6 gap-4 lg:gap-2">
                        ...
                        <div
                            class="flex flex-col gap-3 lg:gap-4 pt-2 border-t border-gray-200 lg:border-t-0 lg:col-span-2 justify-start items-start lg:items-end">
                            @if ($item->status === 'Jatuh Tempo')
                                <div
                                    class="w-max h-max flex items-center gap-2 lg:gap-4 border border-red-600 bg-red-600/15 px-4 lg:px-6 py-2 text-xs lg:text-sm text-nowrap font-medium text-red-600 rounded-full">
                            ...
                            @elseif ($item->status === 'Dikembalikan')
                                <div
                                    class="w-max h-max flex items-center gap-2 lg:gap-4 border border-green-600 bg-green-600/15 px-4 lg:px-6 py-2 text-xs lg:text-sm text-nowrap font-medium text-green-600 rounded-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round">
                            ...
                            @else
                                <div
                                    class="w-max h-max flex items-center gap-2 lg:gap-4 border border-blue-600 bg-blue-600/15 px-4 lg:px-6 py-2 text-xs lg:text-sm text-nowrap font-medium text-blue-600 rounded-full">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round">
                                        <circle cx="12" cy="12" r="10" />
